// GENERATED from FIFA World Cup 26 Regulations (May 2026), Annexe C.
// Source: https://digitalhub.fifa.com/m/636f5c9c6f29771f/original/FWC2026_regulations_EN.pdf
// Columns are destinations paired with group winners: 1A, 1B, 1D, 1E, 1G, 1I, 1K, 1L.

export const THIRD_PLACE_DESTINATIONS = [
  "1A",
  "1B",
  "1D",
  "1E",
...
export type ThirdPlaceDestination = (typeof THIRD_PLACE_DESTINATIONS)[number];
export type GroupLabel =
  ...
  | "K"
  | "L";
export type ThirdPlaceCode = `3${GroupLabel}`;
export type ThirdPlaceAssignment = Record<ThirdPlaceDestination, ThirdPlaceCode>;
...
function keyFromCodes(codes: readonly ThirdPlaceCode[]): string {
  return codes
...
export const THIRD_PLACE_LOOKUP: Record<string, ThirdPlaceAssignment> =
  Object.fromEntries(
    RAW_ANNEX_C.trim()
      .split("\n")
      .map((line) => {
        const [, ...codes] = line.trim().split(/\s+/);
        const typedCodes = codes as ThirdPlaceCode[];
        const assignment = Object.fromEntries(
          THIRD_PLACE_DESTINATIONS.map((destination, index) => [
            destination,
            typedCodes[index],
          ]),
        ) as ThirdPlaceAssignment;
        return [keyFromCodes(typedCodes), assignment];
      }),
  );

export function getThirdPlaceAssignments(
  groups: readonly GroupLabel[],
