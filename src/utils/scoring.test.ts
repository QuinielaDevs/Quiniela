...
describe("calculatePredictionMultiplier — distancia en jornadas", () => {
  ...
  it("con piso de referencia en la jornada 1 (pre-torneo, current 0)", () => {
    // Pre-torneo (current 0) la J2 está a 1 de distancia (1.25x) con el piso en J1.
  ...
  it("escala lineal +0.25 por jornada de distancia, tope 2.5x", () => {
    ...
    expect(calculatePredictionMultiplier(2, "group", 1)).toBe(1.25); // dist 1 (ahora 1.25x)
    expect(calculatePredictionMultiplier(3, "group", 1)).toBe(1.5); // dist 2 (ahora 1.50x)
    expect(calculatePredictionMultiplier(3, "group", 2)).toBe(1.25); // J2 en curso (dist 1 -> 1.25x)
  });
