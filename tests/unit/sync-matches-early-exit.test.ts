...
describe("shouldRunSync early exit logic", () => {
  ...
  it("should return true when a match starts in 5 minutes (within 10m window)", async () => {
  ...
  it("should return false when a match starts in 15 minutes (outside 10m window)", async () => {
    // Current time: 12:00 UTC, match kickoff: 12:15 UTC
    const now = new Date("2026-06-12T12:00:00.000Z");
    mockIn.mockResolvedValue({
      data: [
        {
          status: "scheduled",
          match_time: "2026-06-12T12:15:00.000Z",
        },
      ],
      error: null,
    });
  ...
  it("should return true when a match is live and started 120 minutes ago (within T+100m to T+210m window)", async () => {
