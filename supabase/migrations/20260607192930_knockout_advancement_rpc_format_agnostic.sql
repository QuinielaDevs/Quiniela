-- Migracion: knockout_advancement_rpc_format_agnostic
-- Story 7.3 (refinamiento): elimina la dependencia del formato
-- `wc2026:ko:NNN` en la RPC de avance de eliminatorias para que funcione
-- con cualquier `external_ref` de la fila (incluyendo el formato Zafronix
-- `2026-NNN` introducido por la migración del seed a `restoreZafronixData`).
--
-- Antes: `external_ref = format('wc2026:ko:%s', v_slot.bracket_slot)`
-- Ahora: la fila se identifica solo por `bracket_slot` y `stage <> 'group'`,
-- que es suficiente para garantizar unicidad (el constraint
-- `idx_matches_bracket_slot_unique` ya impide duplicados en ese rango).
--
...
comment on function public.fn_admin_apply_knockout_advancement(jsonb) is
  'Aplica participantes resueltos del bracket FIFA 2026 a public.matches. SECURITY DEFINER + admin global; actualiza solo equipos/códigos de slots knockout 73-104 y preserva resultados/estado/fuentes/horarios. Format-agnostic: identifica filas por bracket_slot + stage (no por external_ref).';
