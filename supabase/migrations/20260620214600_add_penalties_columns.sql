-- Migración: add_penalties_columns
-- Agrega columnas penalties_home_score y penalties_away_score a public.matches,
-- y actualiza la función RPC fn_admin_set_match_result para soportarlas.

-- 1. Agregar columnas a matches
alter table public.matches
  add column if not exists penalties_home_score int check (penalties_home_score >= 0),
  add column if not exists penalties_away_score int check (penalties_away_score >= 0);

comment on column public.matches.penalties_home_score is 'Goles anotados en tanda de penales por el equipo local. Nullable.';
comment on column public.matches.penalties_away_score is 'Goles anotados en tanda de penales por el equipo visitante. Nullable.';

-- 2. Eliminar la firma anterior de fn_admin_set_match_result para evitar conflictos
drop function if exists public.fn_admin_set_match_result(uuid, int, int, text);

-- 3. Redefinir fn_admin_set_match_result con los nuevos parámetros de penales
...
begin
  ...
  if p_status in ('live', 'finished') then
    ...
    -- Validaciones para tanda de penales en partidos knockout
    v_pen_home := null;
    v_pen_away := null;
    ...
    if v_match.bracket_slot is not null and p_status = 'finished' then
      if p_home_score = p_away_score then
        if p_penalties_home_score is null or p_penalties_away_score is null then
          raise exception 'Un partido knockout finalizado en empate necesita registrar la tanda de penales para desempatar' using errcode = '22023';
        end if;
        ...
        -- Si no hay empate reglamentario, ignoramos/limpiamos penales
        v_pen_home := null;
        v_pen_away := null;
      end if;
    end if;
...
comment on function public.fn_admin_set_match_result(uuid, int, int, text, int, int) is
  'Fija home_score/away_score/status/penalties de un partido (admin global). Permite empates en eliminatorias si se provee tanda de penales válida y sin empate.';

grant execute on function public.fn_admin_set_match_result(uuid, int, int, text, int, int) to authenticated;
