...
- **Granularidad de errores + observabilidad en Server Actions admin**: `setMatchResult` (y el `toAdminError` compartido de Story 3.3) colapsan todo error no-42501 en `ADMIN_SAVE_ERROR` y el `catch {}` no loguea. P0002 (partido no existe → "refresca") y 22023 (transición/validación inválida) no se distinguen del fallo genérico. Mejora repo-wide del patrón `toAdminError` + logging, no específica de 7.2. [src/app/actions/matches.actions.ts, leagues.actions.ts]
